...
impl OtelObserver {
    ...
    pub fn new(endpoint: Option<&str>, service_name: Option<&str>) -> Result<Self, String> {
        ...
        let service_name = service_name.unwrap_or("mirror");
...
impl Observer for OtelObserver {
    fn record_event(&self, event: &ObserverEvent) {
        let tracer = global::tracer("mirror");
        ...
        match event {
            ...
            // Mirror behavioral events — recorded by the log observer;
            // intentionally no-op here to avoid OTel attribute schema changes.
            ObserverEvent::ProofOfWork { .. }
            | ObserverEvent::CareerRisk { .. }
            | ObserverEvent::BlackboardCircuitBreaker { .. } => {}
        }
    }
...
mod tests {
    ...
    fn test_observer() -> OtelObserver {
        ...
        OtelObserver::new(Some("http://127.0.0.1:19999"), Some("mirror-test"))
