...
/// Parsed proactive OpenRouter credit guard (from flat `Config` fields).
#[derive(Debug, Clone, Copy)]
pub struct OpenRouterProactiveCreditConfig {
...
pub struct Config {
    ...
    /// OpenRouter only: ordered list of model IDs to try if the **requested** model fails with
    /// rate limits, quota/credit errors, or HTTP 402. Put free-tier models here (e.g. `meta-llama/...`)
    /// without changing `default_model` — you can still switch primary models anytime.
    #[serde(default)]
    pub fallback_models: Option<Vec<String>>,
    /// OpenRouter only: when set to **> 0**, poll `GET /api/v1/auth/key` (at most every
    /// `openrouter_proactive_credit_poll_secs`) and if **remaining USD credits** fall **below** this
    /// value, preemptively use the first suitable entry from `fallback_models` instead of the
    /// requested model (error-driven fallback still applies afterward).
    #[serde(default)]
    pub openrouter_proactive_credit_threshold_usd: Option<f64>,
    /// Minimum seconds between proactive OpenRouter key polls (default **120**). Ignored unless
    /// `openrouter_proactive_credit_threshold_usd` is set.
    #[serde(default)]
    pub openrouter_proactive_credit_poll_secs: Option<u64>,
    ...
    /// Mirror sovereign intelligence system configuration.
    #[serde(default)]
    pub mirror: MirrorConfig,
}

// ── Mirror Sovereign Intelligence System ─────────────────────────────────────

/// Configuration for the Mirror multi-agent behavioral intelligence layer.
///
/// All fields are optional with safe defaults so existing configs remain
/// compatible. Enable `mirror.enabled = true` to activate the blackboard
/// pipeline, circuit breaker, and behavioral observability.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MirrorConfig {
    /// Enable the Mirror multi-agent blackboard system (default: false).
    #[serde(default)]
    pub enabled: bool,

    // ── SLM task routing (research §9.3) ────────────────────────────────────
    //
    // Route specific agentic tasks to optimised small language models.
    // Agents choose the route matching their task; falls back to the global
    // default_model when a route is not configured.
    /// Provider + model for continuous background NLP classification.
    /// Research recommendation: Gemma 3 (4B) — ~32 ms edge latency.
    /// Example: provider = "ollama", model = "gemma3:4b"
    #[serde(default)]
    pub journal_slm_provider: Option<String>,
    #[serde(default)]
    pub journal_slm_model: Option<String>,

    /// Provider + model for biometric data synthesis (Health Agent).
    /// Research recommendation: Phi-4-mini (3.8B) — ~45 ms edge latency.
    #[serde(default)]
    pub health_slm_provider: Option<String>,
    #[serde(default)]
    pub health_slm_model: Option<String>,

    /// Provider + model for multi-turn scheduling reasoning (Planner Agent).
    /// Research recommendation: Mistral NeMo (12B) — ~120 ms edge latency.
    #[serde(default)]
    pub planner_slm_provider: Option<String>,
    #[serde(default)]
    pub planner_slm_model: Option<String>,

    /// Provider + model for complex code + competency analysis (Career Agent).
    /// Research recommendation: Phi-4 (14B) — server/high-end GPU.
    #[serde(default)]
    pub career_slm_provider: Option<String>,
    #[serde(default)]
    pub career_slm_model: Option<String>,

    // ── Circuit breaker policy (research §10.2) ──────────────────────────────
    /// Maximum number of automated schedule reshuffles allowed per hour.
    /// Exceeding this rate-limits `PlannerScheduleProposal` events.
    #[serde(default = "default_max_reshuffles_per_hour")]
    pub max_reshuffles_per_hour: u32,

    /// Maximum blast-radius (downstream events affected) before a schedule
    /// proposal is escalated to the user for HITL approval.
    #[serde(default = "default_max_blast_radius")]
    pub max_blast_radius: i64,

    // ── Behavioral observability (research §7.2) ─────────────────────────────
    /// Rolling window in days for computing passive/active proof-of-work ratio.
    #[serde(default = "default_career_window_days")]
    pub career_risk_window_days: u32,

    /// Passive ratio threshold above which a `CareerRisk` event is emitted.
    /// Range: 0.0–1.0 (default 0.7 = 70% passive triggers risk alert).
    #[serde(default = "default_passive_ratio_threshold")]
    pub career_passive_ratio_threshold: f32,
}

fn default_max_reshuffles_per_hour() -> u32 {
    2
}

fn default_max_blast_radius() -> i64 {
    3
}

fn default_career_window_days() -> u32 {
    7
}

fn default_passive_ratio_threshold() -> f32 {
    0.7
}
...
impl Default for MirrorConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            journal_slm_provider: None,
            journal_slm_model: None,
            health_slm_provider: None,
            health_slm_model: None,
            planner_slm_provider: None,
            planner_slm_model: None,
            career_slm_provider: None,
            career_slm_model: None,
            max_reshuffles_per_hour: default_max_reshuffles_per_hour(),
            max_blast_radius: default_max_blast_radius(),
            career_risk_window_days: default_career_window_days(),
            career_passive_ratio_threshold: default_passive_ratio_threshold(),
        }
    }
}
...
/// Configuration for Chain-of-Thought reasoning and Self-Consistency voting
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ReasoningConfig {
    /// Enable Chain-of-Thought prompting for step-by-step reasoning
    #[serde(default = "default_cot_enabled")]
    pub cot_enabled: bool,

    /// Enable Self-Consistency voting (generates multiple reasoning paths)
    #[serde(default)]
    pub self_consistency_enabled: bool,

    /// Number of reasoning paths for self-consistency (3-5 recommended)
    #[serde(default = "default_num_samples")]
    pub num_samples: usize,

    /// Temperature for sampling diverse paths (0.7-1.0 recommended)
    #[serde(default = "default_sampling_temperature")]
    pub sampling_temperature: f64,

    /// Minimum agreement threshold for consensus (0.5-0.7)
    #[serde(default = "default_consensus_threshold")]
    pub consensus_threshold: f32,
}

fn default_cot_enabled() -> bool {
    true // Enable CoT by default for better reasoning
}

fn default_num_samples() -> usize {
    3 // Balance between quality and latency
}

fn default_sampling_temperature() -> f64 {
    0.8 // Higher temperature for diverse reasoning paths
}
...
impl Default for ReasoningConfig {
    fn default() -> Self {
        Self {
            cot_enabled: default_cot_enabled(),
            self_consistency_enabled: false, // Opt-in due to 3x latency
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "mirror".
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `MIRROR_WHATSAPP_APP_SECRET` environment variable
    #[serde(default)]
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to mirror dir)
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mirror_dir = home.join(".mirror");

        Self {
            workspace_dir: mirror_dir.join("workspace"),
            config_path: mirror_dir.join("config.toml"),
            api_key: None,
            ...
            openrouter_proactive_credit_threshold_usd: None,
            openrouter_proactive_credit_poll_secs: None,
...
impl Config {
    pub fn load_or_init() -> Result<Self> {
        ...
        if !mirror_dir.exists() {
            fs::create_dir_all(&mirror_dir).context("Failed to create .mirror directory")?;
            fs::create_dir_all(mirror_dir.join("workspace"))
        ...
        if config_path.exists() {
            ...
            config.workspace_dir = mirror_dir.join("workspace");
    ...
    /// Validate and optionally encrypt an API key before storing it in config.
    /// Returns the key as-is if encryption is disabled, or encrypted if enabled.
    pub fn prepare_api_key_for_storage(
        mirror_dir: &Path,
        api_key: &str,
        encrypt: bool,
    ) -> Result<String> {
        ...
        if api_key.len() < 10 {
            anyhow::bail!("API key appears too short (minimum 10 characters)");
        }

        if encrypt {
            let store = crate::security::SecretStore::new(mirror_dir, true);
            store.encrypt(api_key).context("Failed to encrypt API key")
        } else {
            Ok(api_key.to_string())
        }
    }

    /// When set via `openrouter_proactive_credit_threshold_usd` (> 0), enables proactive model
    /// switching before requests when OpenRouter reports remaining USD credits below the threshold.
    #[must_use]
    pub fn openrouter_proactive_credit_config(&self) -> Option<OpenRouterProactiveCreditConfig> {
        let t = self.openrouter_proactive_credit_threshold_usd?;
        if t <= 0.0 {
            return None;
        }
        let poll_secs = self.openrouter_proactive_credit_poll_secs.unwrap_or(120).max(10);
        Some(OpenRouterProactiveCreditConfig {
            threshold_usd: t,
            poll_secs,
        })
    }
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: MIRROR_API_KEY or API_KEY (generic)
        if let Ok(key) = std::env::var("MIRROR_API_KEY").or_else(|_| std::env::var("API_KEY")) {
            if !key.is_empty() {
        ...
        // Provider: MIRROR_PROVIDER or PROVIDER
        if let Ok(provider) =
            std::env::var("MIRROR_PROVIDER").or_else(|_| std::env::var("PROVIDER"))
        {
        ...
        // Model: MIRROR_MODEL
        if let Ok(model) = std::env::var("MIRROR_MODEL") {
            if !model.is_empty() {
                self.default_model = Some(model);
            }
        }

        // Workspace directory: MIRROR_WORKSPACE
        if let Ok(workspace) = std::env::var("MIRROR_WORKSPACE") {
            if !workspace.is_empty() {
                self.workspace_dir = PathBuf::from(workspace);
            }
        }

        // Gateway port: MIRROR_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("MIRROR_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
        {
        ...
        // Gateway host: MIRROR_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("MIRROR_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
        {
            if !host.is_empty() {
        ...
        // Allow public bind: MIRROR_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("MIRROR_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Temperature: MIRROR_TEMPERATURE
        if let Ok(temp_str) = std::env::var("MIRROR_TEMPERATURE") {
    ...
    pub fn save(&self) -> Result<()> {
        // Encrypt API keys before serialization
        let mut config_to_save = self.clone();
        let mirror_dir = self
            .config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::security::SecretStore::new(mirror_dir, self.secrets.encrypt);

        // Encrypt main API key
        if let Some(ref plaintext_key) = config_to_save.api_key {
            if !crate::security::SecretStore::is_encrypted(plaintext_key) {
                config_to_save.api_key = Some(
                    store
                        .encrypt(plaintext_key)
                        .context("Failed to encrypt main API key")?,
                );
            }
        }
...
"#;
    ...
    fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("mirror_test_config");
    ...
    fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("mirror_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).unwrap();
...
"#;
    ...
    fn env_override_api_key() {
        ...
        std::env::set_var("MIRROR_API_KEY", "sk-test-env-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        std::env::remove_var("MIRROR_API_KEY");
    }
    ...
    fn env_override_provider() {
        ...
        std::env::set_var("MIRROR_PROVIDER", "anthropic");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("anthropic"));

        std::env::remove_var("MIRROR_PROVIDER");
    }
    ...
    fn env_override_model() {
        ...
        std::env::set_var("MIRROR_MODEL", "gpt-4o");
        config.apply_env_overrides();
        assert_eq!(config.default_model.as_deref(), Some("gpt-4o"));

        std::env::remove_var("MIRROR_MODEL");
    }
    ...
    fn env_override_workspace() {
        ...
        std::env::set_var("MIRROR_WORKSPACE", "/custom/workspace");
        config.apply_env_overrides();
        assert_eq!(config.workspace_dir, PathBuf::from("/custom/workspace"));

        std::env::remove_var("MIRROR_WORKSPACE");
    }
    ...
    fn env_override_empty_values_ignored() {
        ...
        std::env::set_var("MIRROR_PROVIDER", "");
        config.apply_env_overrides();
    ...
    fn env_override_gateway_port() {
        ...
        std::env::set_var("MIRROR_GATEWAY_PORT", "8080");
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        std::env::remove_var("MIRROR_GATEWAY_PORT");
    }
    ...
    fn env_override_gateway_host() {
        ...
        std::env::set_var("MIRROR_GATEWAY_HOST", "0.0.0.0");
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        std::env::remove_var("MIRROR_GATEWAY_HOST");
    }
    ...
    fn env_override_temperature() {
        ...
        std::env::set_var("MIRROR_TEMPERATURE", "0.5");
        config.apply_env_overrides();
        assert!((config.default_temperature - 0.5).abs() < f64::EPSILON);

        std::env::remove_var("MIRROR_TEMPERATURE");
    }
    ...
    fn env_override_temperature_out_of_range_ignored() {
        ...
        std::env::set_var("MIRROR_TEMPERATURE", "3.0");
        config.apply_env_overrides();
        assert!(
