...
/// and forwards PRIVMSG messages to the `Mirror` message bus.
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(
            &mut writer,
            &format!("USER {} 0 * :Mirror", self.username),
        )
        .await?;
...
mod tests {
    ...
    fn irc_config_serde_roundtrip() {
        ...
        assert_eq!(parsed.username.as_deref(), Some("mirror"));
