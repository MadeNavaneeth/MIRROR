...
fn parse_command(content: &str) -> Option<Command> {
    ...
    if lower.starts_with("/edit ") {
        let parts: Vec<&str> = content[6..].splitn(2, ' ').collect();
        if parts.len() == 2 {
            return Some(Command::EditFile {
                path: parts[0].to_string(),
                instruction: parts[1].to_string(),
            });
        }
    } else if lower.starts_with("/create ") {
        let parts: Vec<&str> = content[8..].splitn(2, ' ').collect();
        if parts.len() == 2 {
            return Some(Command::CreateFile {
                path: parts[0].to_string(),
                content: parts[1].to_string(),
            });
        }
    } else if lower.starts_with("/browse ") {
        let parts: Vec<&str> = content[8..].splitn(2, ' ').collect();
        ...
        return Some(Command::Browser { action, url });
    } else if lower.starts_with("/project ") {
        let parts: Vec<&str> = content[9..].splitn(2, ' ').collect();
        ...
        return Some(Command::Project { name, template });
    } else if lower.starts_with("build ") || lower.starts_with("create project ") {
        let prefix = if lower.starts_with("build ") { 6 } else { 15 };
        let name = content[prefix..].trim().to_string();
        if !name.is_empty() {
            return Some(Command::Project {
                name,
                template: None,
            });
        }
...
async fn process_channel_message(ctx: Arc<ChannelRuntimeContext>, msg: traits::ChannelMessage) {
    ...
    {
        if let Ok(sessions) = ctx.sessions.read() {
            if let Some(session) = sessions.get(&session_key) {
                active_project = session.active_project.clone();
            }
        }
    }
    ...
    // Prepend active project context
    if let Some(ref project) = active_project {
        enriched_message = format!(
            "Current Working Project: {}\nAll file operations should be relative to: projects/{}\n\n{}",
            project, project, enriched_message
        );
    }
    ...
    if let Some(command) = parse_command(&msg.content) {
        ...
        let response = match command {
            Command::EditFile { path, instruction } => {
                format!("I would edit {} with instruction: {}", path, instruction)
            }
            Command::CreateFile { path, content } => {
                format!(
                    "I would create {} with content length: {}",
                    path,
                    content.len()
                )
            }
            Command::Browser { action, url } => {
                format!("I would browse: {} {:?}", action, url)
            }
            Command::Project { name, template: _ } => {
                // Create project directory
                let project_path = ctx.workspace_dir.join("projects").join(&name);
                if let Err(e) = std::fs::create_dir_all(&project_path) {
                    format!("Failed to create project {}: {}", name, e)
                } else {
                    // Update session
                    if let Ok(mut sessions) = ctx.sessions.write() {
                        sessions.insert(
                            session_key,
                            Session {
                                active_project: Some(name.clone()),
                            },
                        );
                    }
                    format!("Project '{}' created at {}.\nContext switched! I am now working inside this project.", name, project_path.display())
                }
            }
    ...
    match llm_result {
        Ok(Ok((resp, usage))) => {
            ...
            let usage_records: Vec<TokenUsage> = usage;

            println!(
                "  🤖 Reply ({}ms): {}",
                started_at.elapsed().as_millis(),
                truncate_with_ellipsis(&response_text, 80)
            );

            // Record usage
            if let Ok(tracker) =
                crate::cost::CostTracker::new(ctx.cost_config.clone(), &ctx.workspace_dir)
            {
                let tracker: crate::cost::CostTracker = tracker;
                for record in usage_records {
                    let _ = tracker.record_usage(record);
                }
            }

            if let Some(channel) = target_channel.as_ref() {
                if let Err(e) = channel.send(&response_text, &msg.sender).await {
...
pub fn build_system_prompt(
    ...
    if !tools.is_empty() {
        ...
        prompt.push_str("\n## Protocol\n");
        prompt.push_str("Use <tool_call>{\"name\":\"X\",\"arguments\":{...}}</tool_call>\n");
        prompt.push_str("Multiple calls allowed. Respond based on <tool_result>.\n\n");
    }
    ...
    if prompt.is_empty() {
        "You are Mirror, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct.".to_string()
    } else {
        prompt
    }
}
...
pub fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: mirror channel start");
            println!("To check health:    mirror channel doctor");
            println!("To configure:      mirror onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `mirror onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.mirror/config.toml directly");
        }
    }
}
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `mirror onboard` first.");
        return Ok(());
    }

    println!("🩺 Mirror Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `mirror gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    system_prompt.push_str(&build_tool_instructions(tools_registry.as_ref(), &model));
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `mirror onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 Mirror Channel Server");
    println!("  🤖 Model:    {model}");
    println!(
        "  🧠 Memory:   {} (auto-save: {})",
...
{"name":"mock_price","arguments":{"symbol":"BTC"}}
    ...
    fn prompt_injects_tools() {
        ...
        assert!(prompt.contains("memory_recall: Search memory"));
    }
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from Mirror 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
