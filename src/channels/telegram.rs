...
Allowlist Telegram @username or numeric user ID, then run `mirror onboard --channels-only`.",
                    ...
                    let msg = ChannelMessage {
                        ...
                        // Use chat_id as sender so replies work (API requires chat_id, not username)
                        sender: chat_id.clone(),
                        // Prepend username for context since sender is now an ID
                        content: if username != "unknown" {
                            format!("(User: @{username}) {text}")
                        } else {
                            text.to_string()
                        },
