...
pub fn run_wizard() -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to Mirror — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    let (provider, api_key, model, fallback_models) = setup_provider(&workspace_dir)?;

    print_step(3, 9, "Channels (How You Talk to Mirror)");
    ...
    print_step(10, 10, "Finalize & Launch");
    ...
    let config = Config {
        ...
        mirror: crate::config::schema::MirrorConfig::default(),
    };

    setup_proactive(&config)?;
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("MIRROR_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to Mirror)");
...
/// Use `mirror onboard` or `mirror onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `mirror onboard --interactive` for the full wizard.
...
pub fn run_quick_setup(
    ...
    let workspace_dir = mirror_dir.join("workspace");
    ...
    let config = Config {
        ...
        fallback_models: if provider_name == "openrouter" {
            Some(vec![
    ...
    if api_key.is_none() {
        ...
        println!("    2. Or edit:           ~/.mirror/config.toml");
        println!("    3. Chat:              mirror agent -m \"Hello!\"");
        println!("    4. Gateway:           mirror gateway");
    } else {
        println!("    1. Chat:     mirror agent -m \"Hello!\"");
        println!("    2. Gateway:  mirror gateway");
        println!("    3. Status:   mirror status");
    }
    println!();
...
pub fn run_models_refresh(
    ...
    if !force {
        if let Some(cached) = load_cached_models_for_provider(
            ...
            println!();
            println!(
                "Tip: run `mirror models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let default_dir = home.join(".mirror");
...
#[allow(clippy::too_many_lines)]
fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Vec<String>)> {
    // ── Tier selection ──
    let tiers = vec![
    ...
    if providers.is_empty() {
        ...
        print_bullet("Mirror works with ANY API that speaks the OpenAI chat completions format.");
        ...
        return Ok((provider_name, api_key, model, Vec::new()));
    }
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("Mirror will reuse your existing Gemini CLI authentication.");
            println!();
    ...
    if provider_name == "openrouter" {
        let add_fallbacks = Confirm::new()
            .with_prompt("  Add fallback models for OpenRouter (triggers on quota errors)?")
            .default(true)
            .interact()?;

        if add_fallbacks {
            let fallbacks_str: String = Input::new()
                .with_prompt("  Enter fallback models (comma-separated, e.g. google/gemini-2.0-flash, meta-llama/llama-3.3-70b)")
                .allow_empty(true)
                .interact_text()?;

            fallback_models = fallbacks_str
                .split(',')
    ...
    Ok((provider_name.to_string(), api_key, model, fallback_models))
}
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how Mirror connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("Mirror uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("Mirror can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("Mirror can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should Mirror interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how Mirror stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to Mirror from anywhere.");
        ...
        match choice {
            0 => {
                ...
                println!(
                    ...
                    style("— talk to Mirror from Telegram").dim()
                );
            ...
            1 => {
                ...
                println!(
                    ...
                    style("— talk to Mirror from Discord").dim()
                );
            ...
            2 => {
                ...
                println!(
                    ...
                    style("— talk to Mirror from Slack").dim()
                );
            ...
            3 => {
                ...
                print_bullet("Mirror reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            5 => {
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("mirror-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via MIRROR_WHATSAPP_APP_SECRET env var
                    allowed_numbers,
                });
            }
            6 => {
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("Mirror is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if has_channels {
        ...
        println!("       {}", style("mirror channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("mirror agent -m \"Hello, Mirror!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("mirror agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("mirror status").yellow());
...
pub fn run_models_list(config: &Config, free_only: bool, limit: usize) -> Result<()> {
    tracing::info!("🔍 Fetching models from OpenRouter...");
    ...
    let payload: OpenRouterModelResponse = response
        .json()
    ...
    let mut count = 0;
    for model in models {
        // Parse pricing strings to f64, defaulting to 0.0 if "-1" or invalid
        ...
        // Check if free (using a small epsilon)
        let is_free = prompt_cost.abs() < f64::EPSILON && completion_cost.abs() < f64::EPSILON;
        ...
        let prompt_display = if is_free {
            "FREE".to_string()
        } else {
            format!("${:.2}", prompt_cost * 1_000_000.0)
        };

        let completion_display = if is_free {
            "FREE".to_string()
        } else {
            format!("${:.2}", completion_cost * 1_000_000.0)
        };
    ...
    if count == 0 {
        if free_only {
            println!("No free models found.");
        } else {
            println!("No models found.");
        }
    } else {
        println!();
        println!("(Showing {} models)", count);
    }
...
mod tests {
    ...
    fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = fs::read_to_string(tmp.path().join("IDENTITY.md")).unwrap();
        assert!(
            identity.contains("**Name:** Mirror"),
            "should default agent name to Mirror"
        );
    ...
    fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = fs::read_to_string(tmp.path().join("SOUL.md")).unwrap();
        assert!(soul.contains("Mirror-v2"));
    }
...
pub fn setup_proactive(config: &crate::config::Config) -> anyhow::Result<()> {
    if dialoguer::Confirm::new()
        .with_prompt("Enable Proactive FlowState? (Watches behavior + prepares context)")
        .default(true)
        .interact()?
    {
        crate::cron::add_job(config, "*/15 * * * *", "mirror proactive scan")?;
        println!(
            "  {}",
            console::style("✅ Proactive Engine scheduled for every 15 minutes.").green()
        );
    }

    if dialoguer::Confirm::new()
        .with_prompt("Enable Weekly Heartbeat? (Self-improving maintenance)")
        .default(true)
        .interact()?
    {
        crate::cron::add_job(config, "0 0 * * 0", "mirror heartbeat maintain")?;
        println!(
            "  {}",
            console::style("✅ Weekly Heartbeat scheduled for Sundays at midnight.").green()
        );
    }
