...
/// Handle `mirror peripheral` subcommands.
    ...
    match cmd {
        crate::PeripheralCommands::List => {
            ...
            if boards.is_empty() {
                ...
                println!();
                println!("Add one with: mirror peripheral add <board> <path>");
                println!("  Example: mirror peripheral add nucleo-f401re /dev/ttyACM0");
                println!();
