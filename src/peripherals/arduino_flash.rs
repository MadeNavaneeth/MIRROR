//! Flash Mirror Arduino firmware via arduino-cli.
//!
...
/// Mirror Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/mirror-arduino/mirror-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "mirror-arduino";
...
/// Flash Mirror firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("mirror_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling Mirror Arduino firmware...");
    ...
    println!("Mirror firmware flashed successfully.");
