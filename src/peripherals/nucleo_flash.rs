//! Flash Mirror Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash Mirror Nucleo firmware. Builds from firmware/mirror-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (mirror's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("mirror-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from mirror repo root.",
            firmware_dir.display()
        );
    }

    println!("Building Mirror Nucleo firmware...");
    ...
    println!("Mirror Nucleo firmware flashed successfully.");
