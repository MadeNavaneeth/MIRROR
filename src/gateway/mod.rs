...
pub struct AppState {
    ...
    pub storage: Arc<crate::auth::storage::CredentialStorage>,
}
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    let whatsapp_app_secret: Option<Arc<str>> = std::env::var("MIRROR_WHATSAPP_APP_SECRET")
        .ok()
        .and_then(|secret| {
    ...
    println!("🦀 Mirror Gateway listening on http://{display_addr}");
    ...
    let storage = Arc::new(crate::auth::storage::CredentialStorage::new(
    ...
    let app = Router::new()
        ...
        .route("/api/proactive/logs", get(handle_proactive_logs))
        .route("/api/config", get(handle_config).post(handle_update_config))
        .route("/api/skills", get(handle_list_skills))
        .route("/api/cost", get(handle_cost_summary))
        .route("/api/chat", post(handle_chat))
        .route("/api/auth/status", get(handle_auth_status))
        .route(
            "/api/memories",
            get(handle_list_memories).post(handle_store_memory),
        )
        .route("/api/memories/:key", get(handle_get_memory))
        .fallback_service(
...
/// GET /api/proactive/logs — fetch proactive engine logs
async fn handle_proactive_logs(State(state): State<AppState>) -> impl IntoResponse {
    let log_path = state.config.workspace_dir.join("PROACTIVE_LOG.md");
    match tokio::fs::read_to_string(&log_path).await {
        Ok(content) => (StatusCode::OK, content),
        Err(_) => (
            StatusCode::OK,
            "No intelligence logs available yet.".to_string(),
        ),
    }
}

const MASKED_SECRET_VALUE: &str = "***";
const SECRET_STRING_KEYS: &[&str] = &[
    "access_token",
    "api_key",
    "app_secret",
    "app_token",
...
const SECRET_ARRAY_KEYS: &[&str] = &["api_keys", "paired_tokens"];

fn mask_secret_string(value: &mut serde_json::Value) {
    if matches!(value, serde_json::Value::String(text) if !text.is_empty()) {
        *value = serde_json::Value::String(MASKED_SECRET_VALUE.to_string());
    }
}

fn mask_secret_string_array(value: &mut serde_json::Value) {
    if let serde_json::Value::Array(items) = value {
        for item in items {
            mask_secret_string(item);
        }
    }
}

fn sanitize_secret_fields(value: &mut serde_json::Value) {
    match value {
        serde_json::Value::Object(map) => {
            for (key, child) in map {
                if SECRET_STRING_KEYS.contains(&key.as_str()) {
                    mask_secret_string(child);
                    continue;
                }

                if SECRET_ARRAY_KEYS.contains(&key.as_str()) {
                    mask_secret_string_array(child);
                    continue;
                }
...
fn sanitize_config_for_response(config: &Config) -> serde_json::Value {
    match serde_json::to_value(config) {
        Ok(mut value) => {
            sanitize_secret_fields(&mut value);
            value
        }
        Err(_) => serde_json::json!({ "error": "failed to serialize config" }),
    }
}

fn strip_masked_secret_placeholders(value: &mut serde_json::Value) {
    match value {
        serde_json::Value::Object(map) => {
                ...
                if SECRET_STRING_KEYS.contains(&key.as_str()) {
                    !matches!(
                        ...
                        serde_json::Value::String(text)
                            if text.is_empty() || text == MASKED_SECRET_VALUE
                    )
                } else {
                    strip_masked_secret_placeholders(child);
                    true
                }
            });
        }
        serde_json::Value::Array(items) => {
            for item in items {
                strip_masked_secret_placeholders(item);
            }
        }
        _ => {}
    }
...
/// GET /api/config — fetch current configuration (sanitized)
async fn handle_config(State(state): State<AppState>) -> impl IntoResponse {
    Json(sanitize_config_for_response(&state.config))
}

/// GET /api/skills — list installed skills from lock.json
async fn handle_list_skills(State(state): State<AppState>) -> impl IntoResponse {
    let lock_paths = [
        state
            .config
            .workspace_dir
    ...
    (StatusCode::OK, "{\"skills\": {}}").into_response()
}

/// GET /api/cost — fetch cost summary
async fn handle_cost_summary(State(state): State<AppState>) -> impl IntoResponse {
    match state.cost_tracker.get_summary() {
...
/// POST /api/config — update configuration (all sections)
...
pub struct UpdateConfigBody {
    ...
    // Nested sections (accepted as raw JSON values, merged into config)
    ...
    pub composio: Option<serde_json::Value>,
    ...
    pub channels_config: Option<serde_json::Value>,
    pub identity: Option<serde_json::Value>,
    pub peripherals: Option<serde_json::Value>,
...
async fn handle_update_config(
    ...
    if let Some(key) = body.api_key {
        if !key.is_empty() && key != MASKED_SECRET_VALUE {
    ...
    if let Some(temp) = body.default_temperature {
        state.config.default_temperature = temp;
    }

    // Merge nested sections by round-tripping through JSON
    // This is safe: we serialize the current config, patch the section, deserialize back
    let mut cfg_val = match serde_json::to_value(&state.config) {
        Ok(v) => v,
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    for (key, maybe_val) in sections {
        let maybe_val = maybe_val.map(|mut section| {
            strip_masked_secret_placeholders(&mut section);
            section
        });

        if let (Some(new_section), Some(obj)) = (maybe_val, cfg_val.as_object_mut()) {
            if let (Some(existing), Some(new_obj)) = (obj.get_mut(key), new_section.as_object()) {
                // Deep-merge: overwrite only keys present in the incoming section
                ...
                // Section didn't exist yet — insert wholesale
                if let Some(obj) = cfg_val.as_object_mut() {
                    obj.insert(key.to_string(), new_section.clone());
                }
            }
        }
    }
    ...
    match serde_json::from_value::<crate::config::Config>(cfg_val) {
        Ok(mut new_cfg) => {
            // Restore non-serialized runtime fields
            new_cfg.workspace_dir = state.config.workspace_dir.clone();
            new_cfg.config_path = state.config.config_path.clone();
        ...
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({"error": format!("config deserialize: {e}")})),
        )
            .into_response(),
    }
}

/// POST /api/chat — interactive chat endpoint
#[derive(serde::Deserialize)]
pub struct ChatBody {
    pub message: String,
...
async fn handle_chat(
    ...
    match crate::agent::loop_::process_message_with_agent(
        state.config.clone(),
        &body.message,
...
/// GET /api/auth/status — show authenticated providers
async fn handle_auth_status(State(state): State<AppState>) -> impl IntoResponse {
    let providers = state.storage.list_providers();
    Json(serde_json::json!({
...
mod tests {
    ...
    fn test_app_state(
        ...
        let root =
            std::env::temp_dir().join(format!("mirror-gateway-test-{}", uuid::Uuid::new_v4()));
        ...
        let cost_tracker = Arc::new(CostTracker::new(config.cost.clone(), &workspace_dir).unwrap());
        let storage = Arc::new(crate::auth::storage::CredentialStorage::new(
            root.join("credentials.json"),
            Arc::new(crate::security::SecretStore::new(&root, false)),
        ));
    ...
    #[test]
    fn sanitize_config_masks_nested_secret_fields() {
        let mut config = Config::default();
        config.api_key = Some("sk-top-level".into());
        config.reliability.api_keys = vec!["sk-rotated".into()];
        config.composio.api_key = Some("compio-secret".into());
        config.tunnel.cloudflare = Some(crate::config::schema::CloudflareTunnelConfig {
            token: "cf-token".into(),
        });
        config.channels_config.telegram = Some(TelegramConfig {
            bot_token: "telegram-bot-token".into(),
            allowed_users: vec!["mirror_user".into()],
        });

        let sanitized = sanitize_config_for_response(&config);

        assert_eq!(sanitized["api_key"], MASKED_SECRET_VALUE);
        assert_eq!(sanitized["reliability"]["api_keys"][0], MASKED_SECRET_VALUE);
        assert_eq!(sanitized["composio"]["api_key"], MASKED_SECRET_VALUE);
        assert_eq!(
            sanitized["tunnel"]["cloudflare"]["token"],
            MASKED_SECRET_VALUE
        );
        assert_eq!(
            sanitized["channels_config"]["telegram"]["bot_token"],
            MASKED_SECRET_VALUE
        );
        assert_eq!(
            sanitized["channels_config"]["telegram"]["allowed_users"][0],
            "mirror_user"
        );
    }

    #[test]
    fn strip_masked_secret_placeholders_preserves_real_values_only() {
        ...
        assert!(section["composio"].get("api_key").is_none());
        assert!(section["agents"]["planner"].get("api_key").is_none());
        assert_eq!(section["composio"]["entity_id"], "default");
        assert_eq!(section["default_model"], "gpt-5-mini");
    }
...
async fn handle_store_memory(
    ...
    let category = match body.category.as_deref() {
        Some("core") => crate::memory::MemoryCategory::Core,
        Some("daily") => crate::memory::MemoryCategory::Daily,
        Some("conversation") => crate::memory::MemoryCategory::Conversation,
        Some(custom) => crate::memory::MemoryCategory::Custom(custom.to_string()),
        None => crate::memory::MemoryCategory::Daily,
    };

    match state.mem.store(&body.key, &body.content, category).await {
        Ok(_) => (
            StatusCode::OK,
...
async fn handle_list_memories(
    ...
    match state.mem.list(category_filter.as_ref()).await {
