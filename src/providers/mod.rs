...
pub use traits::{
    ChatMessage, ChatRequest, ChatResponse, ChatResponseChunk, ChatStream, ConversationMessage,
    Provider, TokenUsage, ToolCall, ToolResultMessage,
};
...
/// 3. Generic fallback variables (`MIRROR_API_KEY`, `API_KEY`)
///
...
/// Resolve API key from explicit config and environment (same rules as provider factories).
pub fn resolve_api_key(name: &str, api_key: Option<&str>) -> Option<String> {
    ...
    for env_var in ["MIRROR_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
...
pub fn create_provider(
    ...
    openrouter_proactive_credit: Option<crate::config::OpenRouterProactiveCreditConfig>,
    ...
    match name {
        ...
        _ => anyhow::bail!(
            "Unknown provider: {name}. Check README for supported providers or run `mirror onboard --interactive` to reconfigure.\n\
...
pub fn create_routed_provider(
    ...
    for name in &needed {
        let key = model_routes
            ...
            .or(api_key);
        let proactive = if name == primary_name {
            openrouter_proactive_credit
        } else {
            None
        };
        match create_resilient_provider(
            name,
            key,
            reliability,
            cost_config,
...
mod tests {
    ...
    fn factory_openrouter() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("openrouter", Some("sk-test"), &config, vec![], None).is_ok());
        assert!(create_provider("openrouter", None, &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_anthropic() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("anthropic", Some("sk-test"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_openai() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("openai", Some("sk-test"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_ollama() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("ollama", None, &config, vec![], None).is_ok());
        // Ollama ignores the api_key parameter since it's a local service
        assert!(create_provider("ollama", Some("dummy"), &config, vec![], None).is_ok());
        assert!(create_provider("ollama", Some("any-value-here"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_gemini() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("gemini", Some("test-key"), &config, vec![], None).is_ok());
        assert!(create_provider("google", Some("test-key"), &config, vec![], None).is_ok());
        assert!(create_provider("google-gemini", Some("test-key"), &config, vec![], None).is_ok());
        // Should also work without key (will try CLI auth)
        assert!(create_provider("gemini", None, &config, vec![], None).is_ok());
    }
    ...
    fn factory_venice() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("venice", Some("vn-key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_vercel() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("vercel", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("vercel-ai", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_cloudflare() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("cloudflare", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("cloudflare-ai", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_moonshot() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("moonshot", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("kimi", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_synthetic() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("synthetic", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_opencode() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("opencode", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("opencode-zen", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_zai() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("zai", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("z.ai", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_glm() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("glm", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("zhipu", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_minimax() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("minimax", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_bedrock() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("bedrock", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("aws-bedrock", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_qianfan() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("qianfan", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("baidu", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_qwen() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("qwen", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("dashscope", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("qwen-intl", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("dashscope-intl", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("qwen-us", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("dashscope-us", Some("key"), &config, vec![], None).is_ok());
    }
    ...
    fn factory_groq() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("groq", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_mistral() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("mistral", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_xai() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("xai", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("grok", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_deepseek() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("deepseek", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_together() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("together", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("together-ai", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_fireworks() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("fireworks", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("fireworks-ai", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_perplexity() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("perplexity", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_cohere() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("cohere", Some("key"), &config, vec![], None).is_ok());
    }

    #[test]
    fn factory_copilot() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("copilot", Some("key"), &config, vec![], None).is_ok());
        assert!(create_provider("github-copilot", Some("key"), &config, vec![], None).is_ok());
    }
    ...
    fn factory_custom_url() {
        let config = crate::config::CostConfig::default();
        let p = create_provider("custom:https://my-llm.example.com", Some("key"), &config, vec![], None);
        assert!(p.is_ok());
    }
    ...
    fn factory_custom_localhost() {
        let config = crate::config::CostConfig::default();
        let p = create_provider("custom:http://localhost:1234", Some("key"), &config, vec![], None);
        assert!(p.is_ok());
    }
    ...
    fn factory_custom_no_key() {
        let config = crate::config::CostConfig::default();
        let p = create_provider("custom:https://my-llm.example.com", None, &config, vec![], None);
        assert!(p.is_ok());
    }
    ...
    fn factory_custom_empty_url_errors() {
        let config = crate::config::CostConfig::default();
        match create_provider("custom:", None, &config, vec![], None) {
            Err(e) => assert!(
    ...
    fn factory_custom_invalid_url_errors() {
        let config = crate::config::CostConfig::default();
        match create_provider("custom:not-a-url", None, &config, vec![], None) {
            Err(e) => assert!(
    ...
    fn factory_custom_unsupported_scheme_errors() {
        let config = crate::config::CostConfig::default();
        match create_provider("custom:ftp://example.com", None, &config, vec![], None) {
            Err(e) => assert!(
    ...
    fn factory_custom_trims_whitespace() {
        ...
        let p = create_provider(
            "custom:  https://my-llm.example.com  ",
            Some("key"),
            &config,
            vec![],
            None,
    ...
    fn factory_anthropic_custom_no_key() {
        let config = crate::config::CostConfig::default();
        let p = create_provider("anthropic-custom:https://api.example.com", None, &config, vec![], None);
        assert!(p.is_ok());
    }
    ...
    fn factory_anthropic_custom_empty_url_errors() {
        let config = crate::config::CostConfig::default();
        match create_provider("anthropic-custom:", None, &config, vec![], None) {
            Err(e) => assert!(
                e.to_string().contains("requires a URL"),
                "Expected 'requires a URL', got: {e}"
            ),
    ...
    fn factory_anthropic_custom_invalid_url_errors() {
        let config = crate::config::CostConfig::default();
        match create_provider("anthropic-custom:not-a-url", None, &config, vec![], None) {
            Err(e) => assert!(
                e.to_string().contains("requires a valid URL"),
                "Expected 'requires a valid URL', got: {e}"
            ),
    ...
    fn factory_anthropic_custom_unsupported_scheme_errors() {
        let config = crate::config::CostConfig::default();
        match create_provider("anthropic-custom:ftp://example.com", None, &config, vec![], None) {
            Err(e) => assert!(
                e.to_string().contains("http:// or https://"),
                "Expected scheme validation error, got: {e}"
            ),
    ...
    fn factory_unknown_provider_errors() {
        let config = crate::config::CostConfig::default();
        let p = create_provider("nonexistent", None, &config, vec![], None);
        assert!(p.is_err());
    ...
    fn factory_empty_name_errors() {
        let config = crate::config::CostConfig::default();
        assert!(create_provider("", None, &config, vec![], None).is_err());
    }
    ...
    fn resilient_provider_ignores_duplicate_and_invalid_fallbacks() {
        ...
        let cost_config = crate::config::CostConfig::default();

        let provider =
            create_resilient_provider("openrouter", Some("sk-test"), &reliability, &cost_config, vec![], None);
    ...
    fn factory_all_providers_create_successfully() {
        ...
        for name in providers {
            assert!(
                create_provider(name, Some("test-key"), &cost_config, vec![], None).is_ok(),
