...
pub enum ChatResponseChunk {
    /// A fragment of the text content.
    Content(String),
    /// A fragment of a tool call (or complete tool call if provider sends it at once).
    ToolCall(ToolCall),
    /// Token usage report (usually at end of stream).
    Usage(TokenUsage),
}

/// A stream of chat response chunks.
pub type ChatStream = Pin<Box<dyn Stream<Item = anyhow::Result<ChatResponseChunk>> + Send>>;
...
pub trait Provider: Send + Sync {
    ...
    /// Streaming chat API.
    /// Default implementation falls back to non-streaming `chat` and yields a single chunk.
    async fn chat_stream(
        ...
        if let Some(text) = response.text {
            chunks.push(Ok(ChatResponseChunk::Content(text)));
        }
        for tool_call in response.tool_calls {
            chunks.push(Ok(ChatResponseChunk::ToolCall(tool_call)));
        }
        if let Some(usage) = response.usage {
            chunks.push(Ok(ChatResponseChunk::Usage(usage)));
        }
