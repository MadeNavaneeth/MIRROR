...
pub struct DelegateTool {
    ...
    depth: u32,
    /// Cost configuration for sub-agent provider creation
    cost_config: Arc<crate::config::CostConfig>,
}
...
impl Tool for DelegateTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let provider: Box<dyn Provider> =
            match providers::create_provider(
...
mod tests {
    ...
    fn name_and_schema() {
        let tool = DelegateTool::new(
            sample_agents(),
            None,
            Arc::new(crate::config::CostConfig::default()),
        );
    ...
    async fn invalid_provider_returns_error() {
        ...
        let tool = DelegateTool::new(agents, None, Arc::new(crate::config::CostConfig::default()));
        let result = tool
