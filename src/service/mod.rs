...
const SERVICE_LABEL: &str = "com.mirror.daemon";
...
fn start(config: &Config) -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        run_checked(Command::new("systemctl").args(["--user", "start", "mirror.service"]))?;
        println!("✅ Service started");
        Ok(())
    } else {
        let _ = config;
...
fn stop(config: &Config) -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        let _ = run_checked(Command::new("systemctl").args(["--user", "stop", "mirror.service"]));
        println!("✅ Service stopped");
        Ok(())
    } else {
        let _ = config;
...
"#,
    ...
    println!("   Start with: mirror service start");
    Ok(())
}
...
fn install_linux(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "mirror.service"]));
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/mirror.service"));
    }
}
