...
// in `~/.mirror/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.mirror/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(mirror_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: mirror_dir.join(".secret_key"),
            enabled,
        }
    }
