...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `Mirror` workspace
    Openclaw {
...
pub enum PeripheralCommands {
    ...
    /// Flash Mirror firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    Flash {
    ...
    /// Flash Mirror firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
