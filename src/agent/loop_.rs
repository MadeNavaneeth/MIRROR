...
use crate::providers::{self, ChatMessage, ChatRequest, ChatResponseChunk, Provider, ToolCall};
...
fn parse_tool_calls(response: &str) -> (String, Vec<ParsedToolCall>) {
    ...
    // Fall back to XML-style <invoke> tag parsing (Mirror's original format)
...
pub(crate) async fn run_tool_call_loop(
    ...
    let mut collected_usage: Vec<crate::cost::TokenUsage> = Vec::new();
    for _iteration in 0..MAX_TOOL_ITERATIONS {
                    ...
                    match chunk_res {
                        Ok(chunk) => match chunk {
                            ChatResponseChunk::Content(text) => {
                                ...
                                llm_response_text.push_str(&text);
                            }
                            ChatResponseChunk::ToolCall(tc) => tool_calls.push(tc),
                            ChatResponseChunk::Usage(u) => collected_usage.push(u),
                        },
                        Err(e) => {
        ...
        let response_text = llm_response_text;
        let (parsed_text, tools_from_text) = parse_tool_calls(&response_text);

        // If native tool calls were empty, use parsed ones
        if tool_calls.is_empty() {
            tool_calls = tools_from_text
                .into_iter()
                .map(|ptc| ToolCall {
                    id: Uuid::new_v4().to_string(),
                    name: ptc.name,
                    arguments: ptc.arguments.to_string(),
                })
                .collect();
        }
        ...
        // tool_calls is strictly what we use now

        // Execute each tool call and build results
        let tool_futures = tool_calls.iter().map(|call| async move {
            observer.record_event(&ObserverEvent::ToolCallStart {
                tool: call.name.clone(),
            });
            let start = Instant::now();

            // Parse arguments string back to Value
            let args_value: serde_json::Value = serde_json::from_str(&call.arguments)
                .unwrap_or_else(|_| serde_json::Value::String(call.arguments.clone()));

            // ── Mirror Governance ───────────────────────────────────────────
            if config.mirror.enabled {
                if let Ok(blackboard) = BlackboardDb::open(&config.workspace_dir) {
                    let policy = crate::mirror::circuit_breaker::CircuitBreakerPolicy::default();
                    let event_kind = BlackboardEventKind::ToolExecutionProposed;
                    
                    let proposed_event = crate::mirror::blackboard::BlackboardEvent {
                        kind: event_kind.as_str().to_string(),
                        typed_kind: event_kind,
                        id: Uuid::new_v4().to_string(),
                        agent_tag: Some("core".to_string()),
                        ...
                        status: crate::mirror::blackboard::BlackboardEventStatus::Pending,
                        created_at: chrono::Utc::now(),
                    };

                    if let Ok(decision) = policy.evaluate(&proposed_event, &blackboard) {
                        match decision {
                            crate::mirror::circuit_breaker::CircuitBreakerDecision::EscalateToUser { reason } => {
                                observer.record_event(&crate::observability::traits::ObserverEvent::BlackboardCircuitBreaker {
                                    event_kind: "tool_execution_proposed".to_string(),
                                    decision: "blocked".to_string(),
                                    reason: reason.to_string(),
                                });
                                return format!("Execution blocked by Mirror Sovereignty circuit breaker: {reason}");
                            }
                            crate::mirror::circuit_breaker::CircuitBreakerDecision::RateLimit => {
                                observer.record_event(&crate::observability::traits::ObserverEvent::BlackboardCircuitBreaker {
                                    event_kind: "tool_execution_proposed".to_string(),
                                    decision: "rate_limited".to_string(),
                                    reason: "rate limit exceeded".to_string(),
                                });
                                return "Execution blocked by Mirror Sovereignty circuit breaker: rate limit exceeded".to_string();
                            }
                            _ => {}
                        }
                    }
...
/// how to invoke tools.
pub(crate) fn build_tool_instructions(tools_registry: &[Box<dyn Tool>], model: &str) -> String {
    let lite_mode = model.contains("qwen")
        || model.contains("tiny")
        || model.contains("nano")
    ...
    instructions.push_str("Use <tool_call>{\"name\":\"X\",\"arguments\":{...}}</tool_call>\n");
    instructions.push_str("Respond ONLY with actual calls or final answer. No examples/meta.\n\n");
    ...
    for tool in tools_registry {
        if lite_mode && !essential_tools.contains(&tool.name()) {
            continue;
        }
    ...
    if lite_mode {
        instructions.push_str("\n(Note: Some advanced tools are hidden for efficiency. Ask if you need browser/vision/etc.)\n");
    }
...
pub async fn run(
    ...
    let provider: Box<dyn Provider> = providers::create_routed_provider(
        ...
        &config.cost,
        model_name,
        config.fallback_models.clone().unwrap_or_default(),
        config.openrouter_proactive_credit_config(),
    )?;
    ...
    if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
        ...
        tool_descs.push((
            "arduino_upload",
            "Upload agent-generated Arduino sketch. Use when: user asks for 'make a heart', 'blink pattern', or custom LED behavior on Arduino. You write the full .ino code; Mirror compiles and uploads it. Pin 13 = built-in LED on Uno.",
        ));
        tool_descs.push((
            "hardware_memory_map",
    ...
    system_prompt.push_str(&build_tool_instructions(&tools_registry, model_name));
    ...
    if let Some(msg) = message {
        ...
        // ── Mirror Sovereignty Layer ─────────────────────────────────────
        if config.mirror.enabled {
            let class = crate::mirror::classifier::classify_input(&msg);
            let is_open_loop = crate::mirror::classifier::has_open_loop(&msg);
            if let Ok(blackboard) = BlackboardDb::open(&config.workspace_dir) {
                let kind = class.to_blackboard_kind();
                let _ = blackboard.enqueue_typed(
                    kind,
                    Some("journal"),
                    &serde_json::json!({
                        "input": msg
                ...
                if is_open_loop {
                    let _ = blackboard.enqueue_typed(
                        BlackboardEventKind::JournalOpenLoop,
                        Some("journal"),
                        &serde_json::json!({"input": msg}),
                        1,
                        false,
                    );
                }
                observer.record_event(&ObserverEvent::ProofOfWork {
                    activity_type: crate::observability::traits::ProofOfWorkType::PassiveLearning,
                    bloom_level: 2, // Understanding
                    competency_tag: "journal_entry".to_string(),
                    duration_minutes: 1,
                });
            }
        }
        ...
        let response = {
            let resp: std::string::String = res.0;
            let usage: Vec<TokenUsage> = res.1;
            for record in usage {
                let _ = cost_tracker.record_usage(record);
            }
            resp
        };
        ...
        println!("🦀 Mirror Interactive Mode");
        ...
        while let Some(msg) = rx.recv().await {
            ...
            // ── Mirror Sovereignty Layer ─────────────────────────────────
            if config.mirror.enabled {
                let class = crate::mirror::classifier::classify_input(&msg.content);
                let is_open_loop = crate::mirror::classifier::has_open_loop(&msg.content);
                if let Ok(blackboard) = BlackboardDb::open(&config.workspace_dir) {
                    let kind = class.to_blackboard_kind();
                    let _ = blackboard.enqueue_typed(
                        kind,
                    ...
                    if is_open_loop {
                        let _ = blackboard.enqueue_typed(
                            BlackboardEventKind::JournalOpenLoop,
                            Some("journal"),
                            &serde_json::json!({"input": msg.content}),
                            1,
                            false,
                        );
                    }
            ...
            match cost_tracker.check_budget(0.0) {
                Ok(BudgetCheck::Exceeded {
                    current_usd,
                    limit_usd,
                    period,
                }) => {
                    println!("\n❌ Budget exceeded for {period:?}: ${current_usd:.4} (limit: ${limit_usd:.2})");
                    println!("Interaction terminated to prevent additional costs.\n");
                    continue;
                }
                Ok(BudgetCheck::Warning {
                    ...
                    println!("\n⚠️  Budget warning for {period:?}: ${current_usd:.4} (reaches limit: ${limit_usd:.2})\n");
                }
                _ => {}
            }
            ...
            let response = match res {
                Ok(tuple) => {
                    let resp: std::string::String = tuple.0;
                    let usage: Vec<TokenUsage> = tuple.1;
                    for record in usage {
                        let _ = cost_tracker.record_usage(record);
                    }
                    resp
...
pub async fn process_message(config: Config, message: &str) -> Result<String> {
    process_message_with_agent(config, message, None).await
}

pub async fn process_message_with_agent(
    config: Config,
    message: &str,
    agent_id: Option<&str>,
) -> Result<String> {
    // ── Mirror Sovereignty Layer ─────────────────────────────────────────────
    if config.mirror.enabled {
        let class = crate::mirror::classifier::classify_input(message);
        let is_open_loop = crate::mirror::classifier::has_open_loop(message);
        if let Ok(blackboard) = BlackboardDb::open(&config.workspace_dir) {
            let kind = class.to_blackboard_kind();
            let _ = blackboard.enqueue_typed(
                kind,
    ...
    let mut skills = crate::skills::load_skills(&config.workspace_dir);
    let mut skill_content = String::new();

    // If an agent_id is specified, filter the skills to ONLY that one and inject its full content.
    // This enforces the "Exactly 4 agents. No overlap. No ambiguity." rule.
    if let Some(id) = agent_id {
        skills.retain(|s| s.name == id);
        if let Some(s) = skills.first() {
            let location = s.location.clone().unwrap_or_else(|| {
                config
                    .workspace_dir
                    .join("skills")
                    .join(&s.name)
        ...
        if skills.is_empty() {
            anyhow::bail!("Unknown agent/skill: {}", id);
        }
    }
    ...
    if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
        ...
        tool_descs.push((
            "arduino_upload",
            "Upload Arduino sketch. Use for 'make a heart', custom patterns. You write full .ino code; Mirror uploads it.",
        ));
        tool_descs.push((
            "hardware_memory_map",
...
{"name": "file_write", "arguments": {"path": "hello.py", "content": "print('hello')"}}"#;
    ...
    fn build_tool_instructions_includes_all_tools() {
        ...
        let tools = tools::default_tools(security.clone());
        let instructions = build_tool_instructions(&tools, "claude-3-opus");

        assert!(instructions.contains("## Tool Protocol"));
    ...
    #[test]
    fn build_tool_instructions_filters_for_tiny_model() {
        use crate::security::SecurityPolicy;
        let security = Arc::new(SecurityPolicy::from_config(
            &crate::config::AutonomyConfig::default(),
            std::path::Path::new("/tmp"),
        ));
        // default_tools only has shell/file_read/file_write, which are all essential.
        // We need a non-essential tool to test filtering.
        // Let's manually construct a list or use all_tools if possible, but that requires more setup.
        // Instead, let's just use default_tools and verify they are present (since they are essential).
        // Wait, I need to verify something is EXCLUDED.
        // I can use `tools::browser_open::BrowserOpenTool` directly.

        let mut tools = tools::default_tools(security.clone());
        tools.push(Box::new(crate::tools::browser_open::BrowserOpenTool::new(
            security,
            vec![],
        )));

        // Normal model -> includes browser
        let instructions_normal = build_tool_instructions(&tools, "claude-3-opus");
        assert!(instructions_normal.contains("browser_open"));

        // Qwen model -> excludes browser
        let instructions_tiny = build_tool_instructions(&tools, "qwen:0.5b");
        assert!(!instructions_tiny.contains("browser_open"));
        assert!(instructions_tiny.contains("shell")); // Essential
    }
