# Network Deployment — Mirror on Raspberry Pi and Local Network

This document covers deploying Mirror on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | Mirror polls Telegram API; works from anywhere |
...
**Key:** Telegram, Discord, and Slack use **long-polling** — Mirror makes outbound requests. No port forwarding or public IP required.

---

## 2. Mirror on Raspberry Pi
...
- Telegram channel works: Mirror polls Telegram API (outbound)
...
```bash
mirror daemon --host 0.0.0.0 --port 8080
```
...
2. Start a tunnel:
   ...
   ```
   Or use `mirror tunnel` (see tunnel docs).

3. Mirror will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- Mirror calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `mirror daemon` — Telegram channel starts automatically.
...
- [ ] Run `mirror daemon --host 127.0.0.1 --port 8080` (Telegram works without 0.0.0.0)
