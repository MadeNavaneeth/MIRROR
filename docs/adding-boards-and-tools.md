# Adding Boards and Tools — Mirror Hardware Guide

This guide explains how to add new hardware boards and custom tools to Mirror.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.mirror/config.toml)
mirror peripheral add nucleo-f401re /dev/ttyACM0
mirror peripheral add arduino-uno /dev/cu.usbmodem12345
mirror peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, Mirror can index PDF files:
...
2. **Add to config** — `mirror peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `mirror peripheral list` | List configured boards |
| `mirror peripheral add <board> <path>` | Add board (writes config) |
| `mirror peripheral flash` | Flash Arduino firmware |
| `mirror peripheral flash-nucleo` | Flash Nucleo firmware |
| `mirror hardware discover` | List USB devices |
| `mirror hardware info` | Chip info via probe-rs |
