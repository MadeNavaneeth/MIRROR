...
[INFO] ✓ Audit logging enabled (~/.config/mirror/audit.log)
...
Security Status:
  ...
  ✓ Audit logging: ~/.config/mirror/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ mirror config set security.sandbox.enabled false

# Enable specific backend
$ mirror config set security.sandbox.backend firejail

# Adjust limits
$ mirror config set security.resources.max_memory_mb 2048
```
...
```bash
$ mirror onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Status visibility** — `mirror security --status`
