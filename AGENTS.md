# AGENTS.md — Mirror Agent Engineering Protocol
...
Mirror is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use Mirror-native labels only (`MirrorAgent`, `mirror_user`, `mirror_node`).
...
- If identity-like context is unavoidable, use Mirror-scoped roles/labels only (for example: `MirrorAgent`, `MirrorOperator`, `mirror_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `MirrorAgent`, `MirrorOperator`, `MirrorMaintainer`, `mirror_user`
    - service/runtime labels: `mirror_bot`, `mirror_service`, `mirror_runtime`, `mirror_node`
    - environment labels: `mirror_project`, `mirror_workspace`, `mirror_channel`
...
## 13) Self-Improving Memory Steering (Required)

This workspace implements a tiered self-reflection and learning framework. Memory lives in `~/self-improving/`.

- **Learn from Corrections**: When the user corrects your work, log the pattern to `~/self-improving/corrections.md` and evaluate if it should be promoted to `memory.md`.
- **Self-Reflection**: After completing significant tasks, pause and evaluate the outcome. Log lessons to `~/self-improving/reflections.md`.
- **Promotion Rules**: After 3 identical corrections/lessons in 7 days, ask the user to promote the pattern to HOT memory (`memory.md`).
- **Transparency**: Every action influenced by memory MUST cite the source (e.g., "Using X (from projects/foo.md:12)").
