#!/bin/bash
# Mirror Telegram Integration Test Suite
...
if [ -f "target/release/mirror" ]; then
    BINARY_SIZE=$(ls -lh target/release/mirror | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/mirror 2>/dev/null || stat -c%s target/release/mirror)
...
CONFIG_PATH="$HOME/.mirror/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'mirror onboard' first"
    fi
else
    warn "No config file found - run 'mirror onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/mirror channel doctor 2>&1 || true)
END_TIME=$(date +%s)
...
6. [ ] Check logs for errors:
   RUST_LOG=debug mirror channel start
