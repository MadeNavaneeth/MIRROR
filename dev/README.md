...
A fully containerized development sandbox for Mirror agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`mirror-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `mirror` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/mirror-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.mirror` (mounted to `/mirror-data/.mirror`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.mirror` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `mirror-local-ci` container.
